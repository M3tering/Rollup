...
contract Rollup is IRollup {
    ...
    constructor() {
        SSTORE2.writeDeterministic(hex"00", _pointer(this.totalizer.selector, 0));
    ...
    function commitState(
        ...
        SSTORE2.writeDeterministic(totalizeData, _pointer(this.totalizer.selector, chainLength));
        SSTORE2.writeDeterministic(nonceData, _pointer(this.nonce.selector, chainLength));
    }

    function totalizer(uint256 tokenId) external view returns (bytes6) {
        return state(chainLength, this.totalizer.selector, tokenId);
    }
    ...
    function latestStateAddress(uint256 io) external view returns (address) {
        return stateAddress(chainLength, io == 0 ? this.totalizer.selector : this.nonce.selector);
    }
